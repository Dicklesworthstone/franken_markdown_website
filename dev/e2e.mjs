...
function findChrome() {
  ...
  const roots = [`${home}/.cache/ms-playwright`, `${home}/Library/Caches/ms-playwright`];
  const candidates = [];
...
page.on("console", (msg) => {
  ...
  if (/OTS parsing|Failed to decode downloaded font/i.test(msg.text()))
    fontWarnings.push(msg.text());
});
...
check(
  "html download named from title",
  dlHtml.suggestedFilename() === "frankenmarkdown.html",
  dlHtml.suggestedFilename(),
);
...
check(
  "pdf download slugs the title",
  dlPdf.suggestedFilename() === "my_great_doc.pdf",
  dlPdf.suggestedFilename(),
);
await page.locator("[data-sample='code']").click();
...
const maxOn = await page.evaluate(() =>
  document.getElementById("pg-root").classList.contains("pg-max"),
);
...
check(
  "share builds doc-carrying hash",
  /^#view=max&(fmt=pdf&)?z?doc=/.test(shareHash),
  `${shareHash.length} chars`,
);
...
await reader
  ...
  .catch(() => {});
const rtMax = await reader.evaluate(() =>
  document.getElementById("pg-root").classList.contains("pg-max"),
);
...
await reader
  ...
  .catch(() => {});
const rtRendered = await reader.evaluate(() =>
  document.getElementById("pg-html-frame").srcdoc.includes("clean-room highlighter"),
);
...
const docParam = Buffer.from(unicodeMd, "utf8")
  .toString("base64")
  .replace(/\+/g, "-")
  .replace(/\//g, "_")
  .replace(/=+$/, "");
...
await deep
  .waitForFunction(() => document.getElementById("pg-pdf-frame")?.src.startsWith("blob:"), null, {
    timeout: 45000,
  })
  .catch(() => {});
...
check(
  "fmt=pdf deep link opens pdf pane",
  deepState.pdfVisible && deepState.pdfChipStyled && !deepState.htmlChipStyled,
...
check(
  "preview fonts accepted (no OTS rejects)",
  fontWarnings.length === 0,
  fontWarnings.slice(0, 2).join(" | "),
);
