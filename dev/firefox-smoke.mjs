...
const dir = readdirSync(`${process.env.HOME}/.cache/ms-playwright`).find((d) =>
  d.startsWith("firefox-"),
);
const browser = await firefox.launch({
  executablePath: `${process.env.HOME}/.cache/ms-playwright/${dir}/firefox/firefox`,
});
...
await page.waitForFunction(
  () => document.getElementById("pg-html-frame")?.srcdoc?.includes("<h1"),
  null,
  { timeout: 20000 },
);
...
console.log(
  "firefox: pdf path =",
  await page.evaluate(() =>
    document.getElementById("pg-pdf-frame").src.startsWith("blob:")
      ? "inline viewer"
      : "fallback (pdfViewerEnabled=false)",
  ),
);
...
await reader.waitForFunction(
  () => document.getElementById("pg-html-frame")?.srcdoc?.includes("Firefox share test"),
  null,
  { timeout: 60000 },
);
console.log(
  "firefox: share round-trip renders, maximized =",
