...
const fontWarnings = [];
page.on("console", (m) => {
  if (/OTS parsing|Failed to decode downloaded font/i.test(m.text())) fontWarnings.push(m.text());
});
...
console.log(
  ...
  await page.evaluate(
    async () =>
      (await (await fetch("/assets/js/render-worker.js?v=4")).text()).match(/wasm\/[\d.]+/)?.[0],
  ),
);
