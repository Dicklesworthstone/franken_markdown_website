...
const browser = await webkit.launch({
  executablePath: `${process.env.HOME}/.cache/ms-playwright/${dir}/pw_run.sh`,
});
...
console.log(
  "webkit: share hash built,",
  hash.length,
  "chars,",
  hash.includes("zdoc=") ? "compressed" : "plain doc= (no CompressionStream)",
);
const reader = await browser.newPage();
await reader.goto("https://franken-markdown.com/" + hash, { waitUntil: "networkidle" });
await reader.waitForFunction(
  () => document.getElementById("pg-html-frame")?.srcdoc?.includes("WebKit share test"),
  null,
  { timeout: 90000 },
);
console.log(
  "webkit: share round-trip renders, maximized =",
